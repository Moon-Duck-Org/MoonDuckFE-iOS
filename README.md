<div align="center">

# 🐤 문화생활덕후 문덕이

**나만의 문화생활을 한 곳에 기록하는 iOS 앱**

<img src="https://github.com/user-attachments/assets/52f8f11c-4439-41df-8ec5-31f7ee136774" />

<br/>

[![App Store](https://img.shields.io/badge/App_Store-0D96F6?style=for-the-badge&logo=app-store&logoColor=white)](https://apps.apple.com/kr/app/%EB%AC%B8%ED%99%94%EC%83%9D%ED%99%9C%EB%8D%95%ED%9B%84-%EB%AC%B8%EB%8D%95%EC%9D%B4/id6502997117)

📆 **2024.05 - 2024.08** | B-Side 포텐데이 405

</div>

<br/>

## 📱 프로젝트 소개

'문덕이'는 영화, 드라마, 책, 공연 등 다양한 문화생활을 카테고리별로 기록하고 관리할 수 있는 iOS 앱입니다.

### ✨ 주요 특징

| 특징 | 설명 |
|:---:|:---|
| 🎬 **4가지 카테고리** | 영화, 드라마, 책, 공연을 분류하여 체계적으로 관리 |
| 🔍 **Open API 연동** | 영화진흥위원회 API를 활용한 빠른 콘텐츠 검색 |
| 📴 **오프라인 우선** | Realm 기반 로컬 DB로 네트워크 없이도 사용 가능 |
| 🔗 **간편한 공유** | 기록한 문화생활을 주변 사람들과 손쉽게 공유 |

<br/>

## 👤 담당 역할

**iOS 개발** (단독)

- MVP 아키텍처 설계 및 전체 화면 구현
- 4종 Open API 연동 (영화진흥위원회, 네이버, TMDB, 서울시)
- Kakao / Google / Apple 소셜 로그인 구현
- API 기반 → Realm 로컬 DB 마이그레이션

## 👥 팀 구성

| iOS | Backend | Backend |
|:-:|:-:|:-:|
|<img src="https://avatars.githubusercontent.com/u/56523702?v=4" width="120"/>|<img src="https://avatars.githubusercontent.com/u/55906796?v=4" width="120"/>|<img src="https://avatars.githubusercontent.com/u/86522955?v=4" width="120"/>|
|박현선<br/>[@SuniDev](https://github.com/SuniDev)|안정근<br/>[@ajroot5685](https://github.com/ajroot5685)|윤설이<br/>[@Comesfullcircle](https://github.com/Comesfullcircle)|

<br/>

## 🎬 주요 기능

### 인증 및 홈 화면

|로그인|전체 카테고리|필터링/정렬|
|:-:|:-:|:-:|
|<img src="https://github.com/user-attachments/assets/876e0df6-fe46-46ca-837a-1a73f055187d" width="180"/>|<img src="https://github.com/user-attachments/assets/d344afee-ab7c-4ce2-8e17-aa8be0ffde43" width="180"/>|<img src="https://github.com/user-attachments/assets/4931dfc8-0bfc-4b98-ba97-372797279c7d" width="180"/>|
|Kakao, Google, Apple 소셜 로그인|카테고리별 문화생활 기록 조회|최신순, 오래된순, 별점순 정렬|

### 기록 작성

|카테고리 선택|콘텐츠 검색|리뷰 작성|이미지 첨부|
|:-:|:-:|:-:|:-:|
|<img src="https://github.com/user-attachments/assets/f9a5add9-37b9-454b-a31b-1b43ec600a0d" width="150"/>|<img src="https://github.com/user-attachments/assets/4180988a-63b5-48f6-b6e0-6a3bb09da551" width="150"/>|<img src="https://github.com/user-attachments/assets/1281b3cb-92f1-445d-a879-e3b702fd4357" width="150"/>|<img src="https://github.com/user-attachments/assets/d46f26ee-3823-4c63-b55e-b330277bd51f" width="150"/>|

### 상세 조회 및 공유

|상세 조회|공유|마이페이지|설정|
|:-:|:-:|:-:|:-:|
|<img src="https://github.com/user-attachments/assets/df5666f6-2a54-4da7-a123-43cb5a65e0b1" width="150"/>|<img src="https://github.com/user-attachments/assets/795f218f-c542-4080-88c7-c387ec4d0f16" width="150"/>|<img src="https://github.com/user-attachments/assets/6efc7a39-e3e7-4b5f-8b6d-54535e460d88" width="150"/>|<img src="https://github.com/user-attachments/assets/c2039044-e0c2-4856-847a-62d9c0c5d9f9" width="150"/>|

<br/>

## 🛠 기술 스택

### Environment
![iOS](https://img.shields.io/badge/iOS-15.0+-000000?style=flat-square&logo=apple&logoColor=white)
![Xcode](https://img.shields.io/badge/Xcode-15.0+-147EFB?style=flat-square&logo=xcode&logoColor=white)
![Swift](https://img.shields.io/badge/Swift-5.9-F05138?style=flat-square&logo=swift&logoColor=white)

### Framework & Libraries
![UIKit](https://img.shields.io/badge/UIKit-2396F3?style=flat-square&logo=apple&logoColor=white)
![Realm](https://img.shields.io/badge/Realm-39477F?style=flat-square&logo=realm&logoColor=white)
![Alamofire](https://img.shields.io/badge/Alamofire-E4405F?style=flat-square)
![Kingfisher](https://img.shields.io/badge/Kingfisher-5091CD?style=flat-square)

### Authentication
![Kakao](https://img.shields.io/badge/Kakao-FFCD00?style=flat-square&logo=kakao&logoColor=black)
![Google](https://img.shields.io/badge/Google-4285F4?style=flat-square&logo=google&logoColor=white)
![Apple](https://img.shields.io/badge/Apple-000000?style=flat-square&logo=apple&logoColor=white)

### Firebase
![Analytics](https://img.shields.io/badge/Analytics-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![Crashlytics](https://img.shields.io/badge/Crashlytics-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![RemoteConfig](https://img.shields.io/badge/RemoteConfig-FFCA28?style=flat-square&logo=firebase&logoColor=black)

### Code Quality
![SwiftLint](https://img.shields.io/badge/SwiftLint-E4405F?style=flat-square)
![SwiftGen](https://img.shields.io/badge/SwiftGen-5091CD?style=flat-square)
![CocoaPods](https://img.shields.io/badge/CocoaPods-EE3322?style=flat-square&logo=cocoapods&logoColor=white)

<br/>

## 📂 프로젝트 구조

```
MoonDuck/
├── Application/                       # App Lifecycle
│   ├── AppDelegate.swift
│   └── SceneDelegate.swift
│
├── Presentation/                      # MVP - View & Presenter
│   ├── Base/
│   ├── Home/
│   ├── Search/
│   ├── WriteReview/
│   ├── MyInfo/
│   └── Auth/
│
├── Domain/                            # Business Logic
│   ├── Models/                        # Model + Delegate
│   └── Entities/                      # Domain Objects
│
├── Data/                              # Data Layer
│   ├── Network/                       # API Services
│   └── Storage/                       # Realm Storage
│
├── Resources/
│   ├── Generated/                     # SwiftGen
│   └── Assets.xcassets/
│
└── Supporting Files/
```

<br/>

## 📊 성과

| 지표 | 수치 |
|:---:|:---:|
| 🚀 App Store 출시 | 2024년 8월 |
| 📥 다운로드 | 600+ |
| ⭐ 평균 평점 | 5.0 / 5.0 |

<br/>

## 📚 상세 문서

기술적인 내용이 궁금하시다면 아래 문서를 참고해주세요.

| 문서 | 내용 |
|:---|:---|
| [🏗️ 아키텍처 상세](docs/ARCHITECTURE.md) | MVP 패턴, 계층 구조, 데이터 플로우 |
| [🎯 기술적 의사결정](docs/TECHNICAL_DECISIONS.md) | 기술 선택 이유, Open API 통합, 도전과 해결 |
| [🔄 마이그레이션 스토리](docs/MIGRATION_STORY.md) | API → Realm 전환 과정과 성과 |

<br/>

## 🔗 링크

- 📱 [App Store](https://apps.apple.com/kr/app/%EB%AC%B8%ED%99%94%EC%83%9D%ED%99%9C%EB%8D%95%ED%9B%84-%EB%AC%B8%EB%8D%95%EC%9D%B4/id6502997117)
- 🗂️ [레거시 API 버전 (develop-api 브랜치)](https://github.com/Moon-Duck-Org/MoonDuckFE-iOS/tree/develop-api)

<br/>

---

<div align="center">

**Made with ❤️ by Poduck Team**

</div>
